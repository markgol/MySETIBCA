...
// 
//  This module is a copy of the ImageDialog module used in MySETIviewer and customized
//  for this application
//
// This contains the functions ued by the ImageDlg module
//
...
#define MINZOOM 0.5f
#define ZOOMINCR 0.1f
#define MAXZOOM 40.0f
#define WHEELTHRESHOLD 230
...
{
    if (abs(WheelDelta) < WHEELTHRESHOLD) {
        WheelDelta += Delta;
        return;
    }
    WheelDelta = 0;

    // use nonlinear zoom scaling
    // This makes apparent zooming appear more linear to the user

    float newZoom;
    if (scaleFactor >= 1.0f) {
        newZoom = (float)sqrt(scaleFactor);
    }
    else {
        newZoom = 1.0f;
    }

    scaleFactor += (Delta > 0) ? (ZOOMINCR * newZoom) : (-ZOOMINCR * newZoom);
