...
//
//  This contains the dialog callback procedure for Layers dialog box
// 
#include "framework.h"
#include "MySETIBCA.h"
...
INT_PTR CALLBACK SettingsLayersDlg(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        swprintf_s(TempConfig, MAX_PATH, L"%s\\MySETIBCATmp.cfg", szTempDir);
    ...
    case WM_COMMAND:
        ...
        case IDC_ENABLE:
        {
            ...
            if (HIWORD(wParam) == BN_CLICKED || HIWORD(wParam) == BN_DOUBLECLICKED) {
                ...
                if (IsDlgButtonChecked(hDlg, IDC_ENABLE) == BST_CHECKED) {
                    // Layer 0, BCA image can only be enabled if image is loaded
                    if (Selection == 0 && !BCAimageLoaded) {
                        // uncheck the box
                    ...
                    if (Selection == 0) {
                        swprintf_s(szString, MAX_PATH, L"Disabled:  %d iter, %dHx%dV, %s",
                            CurrentIteration, x, y,
                            ImageLayers->LayerFilename[Selection]);
                    }
                    else {
        ...
        case IDC_ADD_LAYER:
        {
            ...
            if (iRes != APP_SUCCESS) {
                MessageMySETIBCAError(hDlg, iRes, L"Add layer failure");
                break;
            }
        ...
        case IDC_DELETE_LAYER:
        {
            ...
            // do not allow deletion of Layer 0, reserved for BCA image
            if (Selection < 0) {
                MessageBox(hDlg, L"No layer selected for deletion", L"Layers", MB_OK);
                return (INT_PTR)TRUE;
            }
            if (Selection == 0) {
                MessageBox(hDlg, L"Can not delete Layer 0", L"Layers", MB_OK);
                return (INT_PTR)TRUE;
            }
        ...
        case IDC_RELOAD:
        {
            ...
            // Save this conifguration in temp dir as "MySETIBCAReload.cfg"
            swprintf_s(TempFile, MAX_PATH, L"%s\\MySETIBCAReload.cfg", szTempDir);
...
void DeleteAllLayers(HWND hDlg)
{
    ...
    // can not delete layer 0, this is reserved for the BCA image
...
void LoadLayerList(HWND hDlg, int CurrentLayer)
{
    ...
    // IDC_LAYER_LIST
    {
        ...
        // layer 0 is handled differently
        // It includes the iteration number for the BCA
        ImageLayers->GetSize(0, &x, &y);
        if (ImageLayers->IsLayerEnabled(0)) {
            swprintf_s(szString, MAX_PATH, L"Enabled:  %d iter, %dHx%dV, %s", CurrentIteration, x, y,
                ImageLayers->LayerFilename[0]);
        }
        else {
            swprintf_s(szString, MAX_PATH, L"Disabled: %d iter, %dHx%dV, %s", CurrentIteration, x, y,
                ImageLayers->LayerFilename[0]);
        }
...
void ApplyLayers(HWND hDlg)
{
    ...
    if (iRes == APP_SUCCESS) {
        ...
        if (iRes != APP_SUCCESS) {
            MessageMySETIBCAError(hDlg, iRes, L"Display 0 gap parameter");
            return;
        }
